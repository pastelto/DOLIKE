	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// ---------------- 페이징 처리 -----------------
				...
				// * maxPage : 총 페이지 수
						 ...
						 */
				maxPage = (int)Math.ceil((double)listCount/adminLimit);
				...
				AdminPageInfo ami = new AdminPageInfo(listCount, currentPage, startPage, endPage, maxPage, pageLimit, adminLimit);
				// 화면에 던져줘야 하기 때문에 객체를 생성한다.
				
				// 다 담아 줬으면 조회를 해야한다.
				ArrayList<Member> list = new AdminService().adminMemberList(ami); //페이지 정보를 가지고 넘길것이다.
				
				request.setAttribute("list", list);
				request.setAttribute("ami", ami); // 페이지 정보를 담음
