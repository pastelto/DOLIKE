...
/**
 * Servlet implementation class MyMessageList
 */
@WebServlet("/mlist.ms")
public class MyMessageList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 나에게 보낸 쪽지 
		listCount = new MessageService().getMyMessageCount(userId);
		...
		ArrayList<Message> list = new MessageService().selectMyMessageList(pi, userId);
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		request.setAttribute("newMsgCount", newMsgCount);
		
		
		RequestDispatcher view = request.getRequestDispatcher("views/message/myMessageListView.jsp");
