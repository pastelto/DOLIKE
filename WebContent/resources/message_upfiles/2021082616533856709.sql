...
DROP TABLE MESSAGE;
DROP TABLE FAVBOARD;
DROP TABLE ADMIN_USER_TABLE CASCADE CONSTRAINTS; 
...
DROP SEQUENCE SEQ_COUNT_ATTA; --첨부파일 번호 시퀀스 
DROP SEQUENCE SEQ_COUNT_REPLY; -- 댓글번호 시퀀스 
DROP SEQUENCE SEQ_COUNT_BOARD; -- 게시글번호 시퀀스 
...
CREATE TABLE CATEGORY (
	...
	CATEGORY_STATUS VARCHAR2(1) DEFAULT 'Y' CHECK (CATEGORY_STATUS IN('Y', 'N'))
);
...
COMMENT ON COLUMN CATEGORY.CATEGORY_STATUS IS '카테고리 상태값(Y/N)';


INSERT INTO CATEGORY 
VALUES (SEQ_CTG.NEXTVAL, '공부', DEFAULT);

INSERT INTO CATEGORY 
VALUES (SEQ_CTG.NEXTVAL, '건강', DEFAULT);

INSERT INTO CATEGORY 
VALUES (SEQ_CTG.NEXTVAL, '여행', DEFAULT);

INSERT INTO CATEGORY 
VALUES (SEQ_CTG.NEXTVAL, '요리', DEFAULT);

INSERT INTO CATEGORY 
VALUES (SEQ_CTG.NEXTVAL, '동물', DEFAULT);

INSERT INTO CATEGORY 
VALUES (SEQ_CTG.NEXTVAL, '패션뷰티', DEFAULT);

INSERT INTO CATEGORY 
VALUES (SEQ_CTG.NEXTVAL, '기타', DEFAULT);
...
CREATE TABLE MEMBER(
	...
	FOREIGN KEY(INTERESTS) REFERENCES CATEGORY(CATEGORY_NAME)
);
...
INSERT INTO MEMBER 
VALUES ('user01', '라이언', 'pass01', '1997-01-01', '010-1111-2222', 'ryan@dolikecom', '무표정사자', NULL, SYSDATE, DEFAULT);
INSERT INTO MEMBER 
VALUES ('user02', '어피치', 'pass02', '1999-12-05', '010-2222-3333', 'apeach@dolikecom', '왕복숭아', NULL, SYSDATE, DEFAULT);
INSERT INTO MEMBER 
VALUES ('user03', '콘', 'pass03', '1990-05-22', '010-3333-4444', 'conn@dolikecom', '초록악어', NULL, SYSDATE, DEFAULT);
...
CREATE TABLE BOARD(
	...
	BOARD_STATUS CHAR DEFAULT 'Y' CHECK (BOARD_STATUS IN ('Y', 'N')),
...
CREATE TABLE BOARD_REPLY(
	...
	FOREIGN KEY (NICKNAME) REFERENCES MEMBER(USER_ID),
	FOREIGN KEY (REPLY_BNO) REFERENCES BOARD(BOARD_NO)
);
...
CREATE TABLE BOARD_ATTACHMENT(
	...
	B_NO NUMBER,
	B_STATUS CHAR DEFAULT 'Y' CHECK (B_STATUS IN ('Y', 'N')), 
	FOREIGN KEY (B_NO) REFERENCES BOARD(BOARD_NO)
);
...
COMMENT ON COLUMN BOARD.BOARD_STATUS IS '글상태';
COMMENT ON COLUMN BOARD.TAG IS '말머리';

COMMENT ON COLUMN BOARD_REPLY.REPLY_NO IS '댓글번호';
COMMENT ON COLUMN BOARD_REPLY.NICKNAME IS '댓글작성자';
COMMENT ON COLUMN BOARD_REPLY.REPLY_CONTENT IS '댓글내용';
COMMENT ON COLUMN BOARD_REPLY.REPLY_BNO IS '글번호';
COMMENT ON COLUMN BOARD_REPLY.REPLY_STATUS IS '댓글상태';

COMMENT ON COLUMN BOARD_ATTACHMENT.B_FILENO IS '첨부파일번호';
...
COMMENT ON COLUMN BOARD_ATTACHMENT.B_NO IS '글번호';
COMMENT ON COLUMN BOARD_ATTACHMENT.B_STATUS IS '첨부파일상태';

CREATE SEQUENCE SEQ_COUNT_BOARD NOCACHE;
CREATE SEQUENCE SEQ_COUNT_REPLY NOCACHE;
...
CREATE TABLE MESSAGE (
	...
	MSG_DEL VARCHAR2(2) DEFAULT 'N' CHECK (MSG_DEL IN ('Y', 'N', 'B')),
	MSG_SDEL VARCHAR2(2) DEFAULT 'N' CHECK (MSG_SDEL IN ('Y', 'N')),
...
COMMENT ON COLUMN MESSAGE.MSG_SDEL IS '보낸쪽지삭제여부';
...
CREATE TABLE MSG_ATTACHMENT (
    ...
    MAT_STATUS VARCHAR2(2) DEFAULT 'N' CHECK (MAT_STATUS IN ('Y', 'N')),
    MAT_SSTATUS VARCHAR2(2) DEFAULT 'N' CHECK (MAT_SSTATUS IN ('Y', 'N')),
...
COMMENT ON COLUMN MSG_ATTACHMENT.MAT_STATUS IS '첨부파일삭제여부';
COMMENT ON COLUMN MSG_ATTACHMENT.MAT_SSTATUS IS '보낸첨부파일삭제여부';
...
CREATE TABLE CH_VOTE (
	...
	VOTE_COUNT	NUMBER,
    VOTE_CONTENT VARCHAR2(1000) NOT NULL,
	CATEGORY_NO	NUMBER		NOT NULL,
...
COMMENT ON COLUMN CH_VOTE.VOTE_CONTENT IS '투표설명';
...
CREATE OR REPLACE TRIGGER TRG_IN_CH_USER AFTER 
 INSERT ON CHALLENGE_USER
 FOR EACH ROW
BEGIN
   UPDATE CHALLENGE
   SET CH_USERCOUNT = CH_USERCOUNT+1
   WHERE CH_NO = :NEW.CH_NO;
END;
/


CREATE OR REPLACE TRIGGER TRG_IN_CH_RP AFTER
 INSERT ON CHALLENGE_REPLY
 FOR EACH ROW
BEGIN
 UPDATE CHALLENGE_USER
 SET RP_COUNT = RP_COUNT+1
 WHERE CH_USER = :NEW.RP_USER;
END;
/


INSERT INTO CHALLENGE_APPLY VALUES (SEQ_AP_NO.NEXTVAL, '여름 여행에 관련된 챌린지가 필요해요!',TO_DATE(20210701, 'yy/mm/dd'), 'user01', 3);
INSERT INTO CH_VOTE VALUES('너의 여름을 부탁해 <나만의 여름 자랑하기>', SEQ_AP_NO.CURRVAL, TO_DATE(20210725,'yy/mm/dd'), TO_DATE(210731,'yy/mm/dd'), 500, '놀러가기 좋은 여름에도 할 수 있는 것은 집콕뿐! 
무더위를 피해 바다로! 산으로! 워터파크로! 떠났던 여러분의 즐거운 여름을 마음껏 자랑해주세요!', 3);
INSERT INTO CHALLENGE VALUES(SEQ_CH_NO.NEXTVAL, '너의 여름을 부탁해 <나만의 여름 자랑하기>', 500, TO_DATE(20210801, 'yy/mm/dd'), TO_DATE(20210808,'yy/mm/dd'), 'Y', '놀러가기 좋은 여름에도 할 수 있는 것은 집콕뿐! 
무더위를 피해 바다로! 산으로! 워터파크로! 떠났던 여러분의 즐거운 여름을 마음껏 자랑해주세요!', 0, 3);
INSERT INTO CH_ATTACHMENT VALUES(SEQ_CHAT_NO.NEXTVAL, '2021082523194853527.png', '지난-여름-001.png', 'C:\BACKEND\DOLIKE\WebContent\resources\challenge_upfiles', SEQ_CH_NO.CURRVAL);
